...
describe ATH::Response::Headers do
  describe "#initialize" do
    it "sets the date on creation" do
      headers = ATH::Response::Headers.new
    ...
    it "copies multiple headers with the same key" do
      headers = ATH::Response::Headers{"foo" => ["one", "two", "three"]}
      headers["foo"].should eq "one,two,three"
    end

    it "sets the proper `cache-control` header based on the provided HTTP::Headers object" do
      headers = ATH::Response::Headers.new HTTP::Headers{"expires" => "Sat, 10 Apr 2021 15:14:59 GMT"}
      headers["cache-control"].should eq "private, must-revalidate"

      headers = ATH::Response::Headers.new HTTP::Headers{"expires" => "Sat, 10 Apr 2021 15:14:59 GMT", "cache-control" => "max-age=3600"}
  ...
  describe "#date" do
    ...
    it "with an invalid datetime string" do
      ATH::Response::Headers{"date" => "foo"}.date.should be_nil
    end
  end

  describe "#delete" do
    it "deletes the provided header" do
      headers = ATH::Response::Headers{"foo" => "bar"}
    ...
    it "removes cache-control header" do
      headers = ATH::Response::Headers{"expires" => "Sat, 10 Apr 2021 15:14:59 GMT"}
  ...
  describe "cache-control" do
    ...
    it "uses what's provided if provided" do
      headers = ATH::Response::Headers{"cache-control" => "public"}
      headers["cache-control"].should eq "public"
      headers.has_cache_control_directive?("public").should be_true
    end

    it "does not add anything if an etag is included" do
      headers = ATH::Response::Headers{"etag" => "abc123"}
      headers["cache-control"].should eq "no-cache, private"
      headers.has_cache_control_directive?("private").should be_true
      headers.has_cache_control_directive?("no-cache").should be_true
    ...
    it "includes special directive with last-modified header" do
      ATH::Response::Headers{"expires" => "Sat, 10 Apr 2021 15:14:59 GMT"}["cache-control"].should eq "private, must-revalidate"
      ATH::Response::Headers{"last-modified" => "Sat, 10 Apr 2021 15:14:59 GMT"}["cache-control"].should eq "private, must-revalidate"
      ATH::Response::Headers{"last-modified" => "Sat, 10 Apr 2021 15:14:59 GMT", "etag" => "abc123"}["cache-control"].should eq "private, must-revalidate"
      ATH::Response::Headers{"last-modified" => "Sat, 10 Apr 2021 15:14:59 GMT", "expires" => "Sat, 10 Apr 2021 15:14:59 GMT"}["cache-control"].should eq "private, must-revalidate"
    end

    it "adds 'private' to existing cache-control header that doesn't have private or public" do
      ATH::Response::Headers{"expires" => "Sat, 10 Apr 2021 15:14:59 GMT", "cache-control" => "max-age=3600"}["cache-control"].should eq "max-age=3600, private"
      ATH::Response::Headers{"cache-control" => "max-age=3600", "expires" => "Sat, 10 Apr 2021 15:14:59 GMT"}["cache-control"].should eq "max-age=3600, private"
    end

    it "does not add private or public with s-maxage" do
      ATH::Response::Headers{"cache-control" => "s-maxage=100"}["cache-control"].should eq "s-maxage=100"
    end

    it "does not alter with multiple directives" do
      ATH::Response::Headers{"cache-control" => "private, max-age=100"}["cache-control"].should eq "private, max-age=100"
      ATH::Response::Headers{"cache-control" => "public, max-age=100"}["cache-control"].should eq "public, max-age=100"
    end
