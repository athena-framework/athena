...
struct MailboxListHeaderTest < ASPEC::TestCase
  ...
  def test_addresses : Nil
    AMIME::Header::MailboxList
      .new("from", addresses = [AMIME::Address.new "me@example.com", %(Jon Sno, "with love")])
      .address_strings
  ...
  def test_quotes_escaped_chars : Nil
    AMIME::Header::MailboxList
      .new("from", addresses = [AMIME::Address.new "me@example.com", %(Jon Sno, \\escaped\\)])
      .address_strings
  ...
  def test_quotes_paren : Nil
    AMIME::Header::MailboxList
      .new("from", addresses = [AMIME::Address.new "me@example.com", %(Jon (Sno))])
      .address_strings
      .should eq [%("Jon (Sno)" <me@example.com>)]
  end
  ...
  def test_utf8_in_domain : Nil
    AMIME::Header::MailboxList
      .new("from", addresses = [AMIME::Address.new "me@fußball.com"])
      .address_strings
  ...
  def test_utf8_in_local_part : Nil
    AMIME::Header::MailboxList
      .new("from", addresses = [AMIME::Address.new "fußball@example.com"])
      .address_strings
  ...
  def test_multiple_addresses : Nil
    AMIME::Header::MailboxList
      .new("from", addresses = [AMIME::Address.new("me@example.com"), AMIME::Address.new("you@example.com")])
      .address_strings
      .should eq ["me@example.com", "you@example.com"]
  end

  def test_encoded_non_ascii : Nil
    header = AMIME::Header::MailboxList.new("sender", [AMIME::Address.new "me@example.com", %(Jon S\x8Fo)])
    header.charset = "iso-8859-1"
    header.address_strings.should eq [%(Jon =?iso-8859-1?Q?S=8Fo?= <me@example.com>)]
  end
