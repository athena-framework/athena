# Processes an `ATH::View` into an [AHTTP::Response](/HTTP/Response) of the proper format.
#
...
module Athena::Framework::View::ViewHandlerInterface
  ...
  # Handles the conversion of the provided *view* into an [AHTTP::Response](/HTTP/Response).
  #
  # If no *request* is provided, it is fetched from [AHTTP::RequestStore](/HTTP/RequestStore).
  abstract def handle(view : ATH::ViewBase, request : AHTTP::Request? = nil) : AHTTP::Response

  # Creates an [AHTTP::Response](/HTTP/Response) based on the provided *view* that'll redirect to the provided *location*.
  #
  ...
  # Creates an [AHTTP::Response](/HTTP/Response) based on the provided *view* and *request*.
